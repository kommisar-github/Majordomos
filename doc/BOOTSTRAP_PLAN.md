...
- `Q-MAJ-LOCATION:` **RESOLVED** — its own repo: **`github.com/kommisar-github/Majordomos`**. `/scm` owns that repo. **Open sub-question (Q-APP-VENDORING):** how does the standalone-app code (`mcp-task-router-app/`, which `/app` + `/ha` own) land in `Majordomos` — **(a)** copied/vendored in (simplest; the app becomes part of this repo), or **(b)** a git submodule / npm dependency pointing back at the seed repo's app (stays in sync, more wiring)? Affects whether `/app`/`/ha`'s owned paths are first-class files in `Majordomos` or a submodule.
