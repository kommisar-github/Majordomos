...
| `feedback/local_model_harness_feedback.md` | reference | `/pm` | `/ops`, `/app` | Task Router maintainer feedback on local-model backends + the pi/opencode harness (seed v4.32, `harness-runner.js`/`local-runner.js`). 6 HIGH (no diff for the `/review` gate; C-1 SSRF not rebind-safe; no harness preflight; CLI exit status never reaches PM ⇒ failed run looks successful; every harness dispatch is a cold start; **doc cannot stand up the agent it governs — loaded only once the agent exists, and mandatory rule 2 is undiscoverable + defaults open**), 6 MEDIUM (unbounded stdout buffer ⇒ supervisor OOM; payload replaces the system prompt; pi/opencode not substitutable at the result-contract layer; +3), 2 LOW, 3 doc bugs, 1 addition. Doc+source review, **zero runtime experience**. Three `/review` rounds + clean-room `peer-reviewer`; round 3 caught a false claim (runners *do* self-register) that was struck before delivery. |
