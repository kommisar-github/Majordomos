...
| **/app** | If the executor is exposed as a route, wire it **loopback-only** into `serverHost` (G3 owner-endpoint tier; attach to live server, no double-start / 409). If it is a CLI, no server change | `mcp-task-router-app/src/serverHost.js` |
